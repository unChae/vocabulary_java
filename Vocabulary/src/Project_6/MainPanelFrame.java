...
public class MainPanelFrame implements ActionListener, MouseListener {
	...
	MainPanelFrame(MainFrame f) {
		...
		for (int i = 0; i < 4; i++) {
			f.lblInforSide[i] = new JLabel();
			f.pnlSide.add(f.lblInforSide[i]);
			f.lblInforSide[i].setForeground(Color.LIGHT_GRAY);
			f.lblInforSide[i].setBounds(14, 85 + i * 25, 258, 18);
		}

		f.lblInforSide[0].setText("Name / ");
		f.lblInforSide[1].setText("Email / ");
		f.lblInforSide[2].setText("Count / ");
		f.lblInforSide[3].setText("Memo");
