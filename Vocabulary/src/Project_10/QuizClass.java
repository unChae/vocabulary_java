...
public class QuizClass {
	...
	public void randExam() {
		...
		for (int i = 0; i < 3; i++) {
			try {
				...
				String rsHiragana = "NULL";
				String rsMean = "NULL";
				String query = "SELECT NVL(KANJI, 'NULL'), HIRAGANA, MEAN FROM (SELECT * FROM VOCA ORDER BY dbms_random.VALUE) WHERE ROWNUM = 1 AND SHAPE = '"
						+ format + "' AND USER_NUMBER = '" + data.userNumber + "'";
				Svr.stmt = Svr.con.prepareStatement(query);
				Svr.rs = Svr.stmt.executeQuery();
				while (Svr.rs.next()) {
				...
				System.out.println("순서 : " + rsKanji + " " + rsHiragana + " " + rsMean);
				boolean bln = true;
				if (i > 0) {
					for (int j = 0; j < 3; j++) {
						if ((rsKanji.equals(strExam[j][0]) && rsHiragana.equals(strExam[j][1]))) {
							bln = false;
						}
					}
					if ((rsKanji.equals(strQuiz[data.cntQuit - 1][1])
							&& rsHiragana.equals(strQuiz[data.cntQuit - 1][2]))) {
						bln = false;
					}
					if (bln == false) {
						bln = true;
			...
			System.out.println("순서 : " + strExam[i][0] + " " + strExam[i][1] + " " + strExam[i][2] + "1");
		}
		randomExam();
	}
	...
	public void quizType() {
		System.out.println(strQuiz[data.cntQuit - 1][1]);
		if (strQuiz[data.cntQuit - 1][1].equals("NULL")) {
			int randNumber = (int) (Math.random() * 2) + 5;
			strQuiz[data.cntQuit - 1][0] = Integer.toString(randNumber);
		}
		// 한자 - > 의미
	...
	public void nextQuestion() {
		if (data.cntQuit < quizCount) {
			...
			f.lblQuizTime.setText(data.cntQuit + "/" + quizCount);
			f.lblQuizNumber.setText("No." + data.cntQuit);
